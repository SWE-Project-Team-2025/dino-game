...
POWER_UP_SPAWN_FREQ_NORMAL = (200, 400)  # Normal game spawning
...
class PowerUpState:
    ...
    def draw_powerup_indicators(self, screen, dino_rect):
        """Draw shield circle and progress bar indicators"""
        if self.has_shield:
            shield_radius = max(dino_rect.width, dino_rect.height) // 2 + 20
            ...
            screen.blit(
                shield_surface,
                (dino_rect.centerx - shield_radius, dino_rect.centery - shield_radius),
            )

            # Draw progress bar above the shield
            progress_bar_width = 60
            progress_bar_height = 8
            progress_bar_x = dino_rect.centerx - progress_bar_width // 2
            progress_bar_y = dino_rect.centery - shield_radius - 15
            ...
            # Progress fill based on remaining shield time
            progress_ratio = self.shield_timer / SHIELD_DURATION
            progress_fill_width = int(progress_bar_width * progress_ratio)
    ...
    def _try_spawn_powerup(self, obstacles):
        ...
        if can_spawn:
            ...
            if random.random() > 0.5:
                self.power_ups.append(ShieldPowerUp())
            else:
